...
So, every time a user makes an intial request to the app or an app path, the server should render the page and send it to the client.
From that moment on, the client can take over control and proceed with rendering the content.
This bridges the gap to initialize the client while the user can consume the initial server-side rendered page.
...
React.js makes heavy use of https://jsx.github.io/[JSX^], a new JavaScript dialect, mixed with HTML elements.
...
_(Some people might be reminded of JSP when reading the code above...)_

JSX will be transpiled (transformed) to executable JavaScript (ES5) by using https://babeljs.io[Babel.js^].
(Formerly this was done by `JSXTransformer`, a library from Facebook, but they switched to Babel, b/c it's more powerful.)
...
CAUTION: Ensure to use the original code from the above mentioned repository when building applications with a React-based ViewEngine. These are much more flexible and powerful than this simplified example!
...
there will be no HTML code visible in the `<div id="content"/>` element if you look at the initial source code received from the server.
...
TIP: Use two different browsers and play with the application. Have a look at the application updating itself every 5 seconds and how the source code relates to these updates (or not).
