...
Nashorn comes along with quite a few language and API extensions, in addition to the ECMAScript standard.
...
Accessing overridden members in JavaScript is traditionally awkward because Java's `super` keyword doesn't exist in ECMAScript.
...
The syntax of overriding members is borrowed from Java's anonymous objects.
